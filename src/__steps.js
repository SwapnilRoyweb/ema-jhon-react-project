// 1. create a firebase project
// 2. create web app
// 3. npm i firebase
// 4. save firebase config and export default app
// 5. firebase site: build> authentication > get started >sign in method
// 6. create sign up and login route

// 1. create a context provider
// 2. create a context and set provider
// 3. set the children props
// 4. set context value
// 5. set the provider

// Hosting
// 1. one time per PC install it
// 2. firebase login

// for each project one time
// 1. firebase init
// 2. proceed
// 3. hosting: firebase(up and down) use space bar for select
// 4.existing project
// 5. select the project carefully
// 6. which project as public directory : dist for vite and build for rect app
// 7. single page application: yes

// for every time deploy
// 1. npm run build
// 2. firebase deploy